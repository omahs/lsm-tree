...
test_singleKeyRun =  do
    let !k = SerialisedKey' (P.fromList [37])
        !e = InsertWithBlob (SerialisedValue' (P.fromList [48, 19])) (55, 77)
    ...
    addRes @=? Nothing
    mp @=? Just (paSingleton k e, [])
    isJust mc @? "expected a chunk"
    Bloom.elem k b @=? True
    Index.search k cix @=? Index.SinglePage 0
...
fromListPageAcc :: [(SerialisedKey, Entry SerialisedValue (Word64, Word32))] -> PageAcc
fromListPageAcc = fromListPageAcc' 0

fromListPageAcc' :: Int -> [(SerialisedKey, Entry SerialisedValue (Word64, Word32))] -> PageAcc
...
fromProtoKOp ::
  ...
  -> (SerialisedKey, Entry SerialisedValue (Word64, Word32))
...
fromProtoBlobRef :: Proto.BlobRef -> (Word64, Word32)
fromProtoBlobRef (Proto.BlobRef x y) = (x, y)
...
getRealKOps :: PageLogical' -> [(SerialisedKey, Entry SerialisedValue (Word64, Word32))]
