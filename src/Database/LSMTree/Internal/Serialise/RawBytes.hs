...
--
-- PRECONDITION: the byte-size @n@ of the raw bytes should be at least 4 bytes.
--
-- PRECONDITION: The raw bytes should be large enough that we can slice out 4
-- bytes after of the bit-offset @off, since we can only slice out bits that are
...
unsafeFromByteString (BS.Internal.BS (GHC.ForeignPtr _ contents) n) =
    case contents of
      ...
      PlainPtr mba# -> RawBytes $ P.Vector 0 n $ ByteArray (unsafeCoerce# mba#)
